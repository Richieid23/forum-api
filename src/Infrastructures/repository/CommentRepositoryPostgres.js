...
class CommentRepositoryPostgres extends CommentRepository {
  ...
  async verifyCommentOwner(comment, owner) {
    ...
    if (result.rowCount === 0) {
      throw new AuthorizationError(
        'anda tidak bisa menghapus komentar orang lain.',
      );
    }
  }
  ...
  async getCommentsThread(thread) {
    const query = {
      text: 'SELECT comments.id, users.username, comments.date, comments.content, comments.is_deleted FROM comments LEFT JOIN users ON users.id = comments.owner WHERE thread_id = $1 ORDER BY comments.date ASC',
      values: [thread],
    };
